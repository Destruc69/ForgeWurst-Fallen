...
public final class Surround extends Hack {
	...
	public void onUpdate(WUpdateEvent event) {
		...
		if (mc.player.ticksExisted % 2 == 0) {
			mc.playerController.processRightClickBlock(mc.player, mc.world, blockPos1, EnumFacing.DOWN, new Vec3d(0, 0, 0), EnumHand.MAIN_HAND);
			float[] rot = RotationUtils.getNeededRotations(new Vec3d(blockPos1.getX(), blockPos1.getY(), blockPos1.getZ()));
			mc.player.connection.sendPacket(new CPacketPlayer.Rotation(rot[0], rot[1], mc.player.onGround));
		}
		if (mc.player.ticksExisted % 4 == 0) {
			mc.playerController.processRightClickBlock(mc.player, mc.world, blockPos2, EnumFacing.DOWN, new Vec3d(0, 0, 0), EnumHand.MAIN_HAND);
			float[] rot = RotationUtils.getNeededRotations(new Vec3d(blockPos2.getX(), blockPos2.getY(), blockPos2.getZ()));
			mc.player.connection.sendPacket(new CPacketPlayer.Rotation(rot[0], rot[1], mc.player.onGround));
		}
		if (mc.player.ticksExisted % 6 == 0) {
			mc.playerController.processRightClickBlock(mc.player, mc.world, blockPos3, EnumFacing.DOWN, new Vec3d(0, 0, 0), EnumHand.MAIN_HAND);
			float[] rot = RotationUtils.getNeededRotations(new Vec3d(blockPos3.getX(), blockPos3.getY(), blockPos3.getZ()));
			mc.player.connection.sendPacket(new CPacketPlayer.Rotation(rot[0], rot[1], mc.player.onGround));
		}
		if (mc.player.ticksExisted % 8 == 0) {
			mc.playerController.processRightClickBlock(mc.player, mc.world, blockPos4, EnumFacing.DOWN, new Vec3d(0, 0, 0), EnumHand.MAIN_HAND);
			float[] rot = RotationUtils.getNeededRotations(new Vec3d(blockPos4.getX(), blockPos4.getY(), blockPos4.getZ()));
			mc.player.connection.sendPacket(new CPacketPlayer.Rotation(rot[0], rot[1], mc.player.onGround));
		}
